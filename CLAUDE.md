...
Migrations are **hand-written** (not autogenerated). The convention is sequential numbering: `019_...`, `020_...`, etc.

**Creating a new migration:**

1. Create `tm_bot/db/alembic/versions/019_your_description.py` (next number in sequence).
2. Use the previous migration's `revision` as your `down_revision`. Check the latest file to find it.
3. Follow this template:

```python
"""Short description

Revision ID: 019_your_description
...
revision: str = '019_your_description'
down_revision = '018_add_multilingual_names'
branch_labels = None
depends_on = None
...
**Applying migrations** — Alembic **must** be run from `/app` inside the container (the `alembic.ini` uses a path relative to `/app`):
