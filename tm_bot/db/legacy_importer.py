...
def ensure_imported(conn, root_dir: str, user_id: str, source: str) -> None:
    """
    Ensure a given legacy source has been imported for a user.

    Sources: promises | actions | sessions | settings
    """
    if _already_imported(conn, user_id, source):
        return

    # Dependencies (actions/sessions refer to promises)
    if source in ("actions", "sessions"):
        ensure_imported(conn, root_dir, user_id, "promises")

    if source == "promises":
        _import_promises(conn, root_dir, user_id)
        return
    if source == "actions":
        _import_actions(conn, root_dir, user_id)
        return
    if source == "sessions":
        _import_sessions(conn, root_dir, user_id)
        return
    if source == "settings":
        _import_settings(conn, root_dir, user_id)
        return
