...
from llms.llm_model_config import normalize_provider_name
...
def _resolve_fallback_provider(
    ...
    Returns:
        ...
        - resolved_provider: "gemini" | "openai" | "deepseek" | "groq" | "xai" | None
        - reason: optional reason code when provider was auto-adjusted
    """
    if not fallback_enabled:
        return None, None

    provider = normalize_provider_name(requested_fallback or "openai")
    primary = normalize_provider_name(primary_provider)
    if (
        provider == "deepseek"
        and not has_deepseek_key
        and primary in {"gemini", "google"}
    ...
    if provider == "deepseek" and not has_deepseek_key:
        ...
        if has_xai_key:
            return "xai", "deepseek_key_missing"
        if has_gemini_creds:
            return "gemini", "deepseek_key_missing"
    ...
    if provider == "groq" and not has_groq_key:
        ...
        if has_xai_key:
            return "xai", "groq_key_missing"
        if has_gemini_creds:
            return "gemini", "groq_key_missing"
        return None, "groq_key_missing"

    if provider == "xai" and not has_xai_key:
        if has_openai_key:
            return "openai", "xai_key_missing"
        if has_groq_key:
            return "groq", "xai_key_missing"
        if has_deepseek_key:
            return "deepseek", "xai_key_missing"
        if has_gemini_creds:
            return "gemini", "xai_key_missing"
        return None, "xai_key_missing"
    ...
    *,
    ...
    def _maybe_add(provider_key: Optional[str], allow_primary: bool = False) -> None:
        ...
        if not _provider_has_credentials(
            ...
            has_xai_key=has_xai_key,
        ):
            return
        chain.append(key)

    _maybe_add(preferred_provider, allow_primary=True)
    for candidate in ("groq", "xai", "openai", "deepseek", "gemini"):
...
def _resolve_fallback_role_providers(
    ...
    *,
    ...
    """
    provider = normalize_provider_name(requested_fallback)
    avoided = {normalize_provider_name(p) for p in (avoid_providers or set()) if p}
    if not provider:
        return None
    ...
    if provider == "xai":
        if not has_xai_key or "xai" in avoided:
            return None
        return {"router": "xai", "planner": "xai", "responder": "xai"}
    ...
    if provider == "deepseek":
        ...
        elif has_xai_key and "xai" not in avoided:
            structured_provider = "xai"
        return {
...
class LLMHandler:
    def __init__(
        ...
        try:
            ...
            xai_role_model_names = {
                "router": str(cfg.get("LLM_XAI_ROUTER_MODEL") or "grok-4.3"),
                "planner": str(cfg.get("LLM_XAI_PLANNER_MODEL") or "grok-4.3"),
                "responder": str(cfg.get("LLM_XAI_RESPONDER_MODEL") or "grok-4.3"),
            }
            ...
            if self._provider_layer_enabled:
                ...
                self.responder_model = _build_role_model(
                    ...
                    xai_role_model_names["responder"],
                )
                self.chat_model = self.responder_model

                # Live-data responder: xAI with native Grok search, used only when the
                # router flags needs_live_data=True.  Kept separate from the primary
                # provider so it's opt-in per turn rather than global.
                self.live_responder_model = None
                _xai_key = cfg.get("XAI_API_KEY", "")
                if _xai_key:
                    from llms.providers.xai_adapter import XAIProviderAdapter
                    _xai_live_adapter = XAIProviderAdapter({})
                    self.live_responder_model = _xai_live_adapter.build_role_model(
                        "responder",
                        {
                            **base_cfg,
                            "xai_api_key": _xai_key,
                            "xai_base_url": cfg.get("XAI_BASE_URL", "https://api.x.ai/v1"),
                            "xai_model": xai_role_model_names["responder"],
                            "live_search": True,
                            "feature_policy": role_policies["responder"],
                        },
                    )
                ...
                requested_fallback, fallback_autoselect_reason = _resolve_fallback_provider(
                    ...
                    has_xai_key=bool(cfg.get("XAI_API_KEY", "")),
                )
            ...
            self.agent_app = create_routed_plan_execute_graph(
                ...
                live_responder_model=self.live_responder_model,
    ...
    def _initialize_context(self) -> None:
        ...
        self.system_message_router_prompt = (
            ...
            "- Set needs_live_data=true ONLY when answering requires current/verifiable internet facts the model "
            "cannot reliably know from training data. This includes:\n"
            "  * Breaking news, today's events, recent announcements, ongoing incidents\n"
            "  * Fact-checking a specific claim, quote, statistic, or news item the user mentions\n"
            "  * Live prices/scores/weather/standings, latest software or API versions, release dates\n"
            "  * Verifying what an organisation, public figure, or official account recently said or posted\n"
            "  Grok's web_search + x_search are used jointly: x_search is for citing primary sources on X "
            "  (official accounts, journalists, eyewitness posts) to corroborate or correct a factual claim, "
            "  NOT for opinions, vibes, memes, sentiment, or 'what are people saying'.\n"
            "- Set needs_live_data=false for opinion/vibe/sentiment questions, casual chat, coaching, "
            "  and anything answerable from the user's own Xaana data. Default false.\n\n"
    ...
    def get_response_api(
        ...
        try:
            ...
            # General per-user daily call limit (0 = unlimited).
            from llms.user_call_gate import check_general, GENERAL_DAILY_LIMIT
            if GENERAL_DAILY_LIMIT > 0:
                _allowed, _used = check_general(safe_user_id)
                if not _allowed:
                    logger.warning({
                        "event": "general_call_gate_blocked",
                        "user_id": safe_user_id,
                        "used_today": _used,
                        "limit": GENERAL_DAILY_LIMIT,
                    })
                    return {
                        "error": "daily_limit_reached",
                        "function_call": "no_op",
                        "response_to_user": (
                            f"You've reached your daily limit of {GENERAL_DAILY_LIMIT} messages. "
                            "Your limit resets at midnight. Try again tomorrow! 🙏"
                        ),
                    }
    ...
    def get_response_group_safe(
        ...
        try:
            ...
            club_description = str(group_context.get("club_description") or "").strip()
            ...
            if member_status:
                ...
                for member in member_status:
                    ...
                    if username:
                        aliases.append(f"username: @{username}")
            ...
            system_text = "\n".join([
                ...
                "You are a friendly presence in the room, not a silent tool: warmth, humour and small talk are welcome",
                "when they fit the moment. You are not a therapist, a mediator between members, or a general assistant",
                "for work unrelated to the club — for those, point people to a DM or to someone better placed to help.",
                "Keep replies short — one paragraph at most.",
                "",
                "Core behaviour:",
                ...
                *(["- Persian style: use casual, conversational Persian. Avoid bookish phrasing, unnecessary formality, and awkward mixed-language words."]
                ...
                "- Off-topic but friendly chat is fine to join briefly — one warm line, then let the group carry it.",
                ...
                "Ignore Persian honorifics like 'جان' (dear), 'خانم' (Ms.), and 'آقا' (Mr.) when matching names.",
