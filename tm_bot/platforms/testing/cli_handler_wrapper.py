...
class CLIHandlerWrapper:
    ...
    def _create_mock_update_and_context(
        self, text: str, user_id: int, message_type: str = "text"
    ) -> tuple:
        """Create mock Update and CallbackContext for the given input."""
    ...
    async def handle_command(self, command: str, user_id: int, args: list = None) -> Optional[str]:
        ...
        update, context = self._create_mock_update_and_context(text, user_id, "command")
        context.args = args
        if getattr(self.bot, "application", None) and hasattr(self.bot.application, "bot"):
    ...
    async def handle_message(self, text: str, user_id: int) -> Optional[str]:
        ...
        update, context = self._create_mock_update_and_context(text, user_id)
        if getattr(self.bot, "application", None) and hasattr(self.bot.application, "bot"):
            context.bot = self.bot.application.bot
