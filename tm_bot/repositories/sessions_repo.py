...
class SessionsRepository:
    """SQLite-backed sessions repository (with lazy import from legacy CSV)."""
    ...
    def create_session(self, session: Session) -> None:
        ...
        with connection_for_root(self.root_dir) as conn:
            ensure_imported(conn, self.root_dir, user, "sessions")
            ...
            conn.execute(
                ...
                (
                    ...
                    user,
                    p_uuid,
                    str(session.status or "running"),
                    dt_to_utc_iso(session.started_at, assume_local_tz=True) or utc_now_iso(),
                    dt_to_utc_iso(session.ended_at, assume_local_tz=True),
                    int(session.paused_seconds_total or 0),
                    dt_to_utc_iso(session.last_state_change_at, assume_local_tz=True),
                    session.message_id,
                    session.chat_id,
                ),
            )
