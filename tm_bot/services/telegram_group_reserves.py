...
A reserve holds no human member: the operator creates the group, grants the
bot admin rights, and leaves. Telegram keeps the bot's rights intact in an
ownerless supergroup, so the club creator can be promoted with nobody to hand
off from. That removes the caretaker-departure window entirely.
"""
...
async def inspect_reserve(bot: Bot, chat_id: int) -> dict[str, Any]:
    """Check the empty, bot-only group and its rights before registration/allocation."""
    ...
    if member_count != 1:
        raise ReserveValidationError("Reserve must contain only the Xaana bot; the operator must leave first")
    if not chat.title:
...
async def register_reserve(bot: Bot, chat_id: int, actor_user_id: int, label: str) -> dict[str, Any]:
    ...
    checked = await inspect_reserve(bot, chat_id)
    ...
    return {
        ...
        "status": "available", "primary_link_revoked": True,
    }
...
def _claim_oldest_reserve(club_id: str) -> dict[str, Any] | None:
    ...
    with get_db_session() as session:
        row = session.execute(
            text("""
                SELECT chat_id, label, original_title
...
def _mark_club_connected(club_id: str) -> bool:
    """Finish setup as soon as the creator actually holds admin rights."""
    with get_db_session() as session:
        updated = session.execute(
            text("""
...
async def handle_reserve_join_request(bot: Bot, join_request: Any) -> str:
    ...
    with get_db_session() as session:
        row = session.execute(
            text("""
                SELECT c.club_id, c.owner_user_id, c.telegram_invite_link,
    ...
    for attempt in range(3):
        try:
            ...
            if promoted.status == "administrator":
                # Nobody has to leave: the group was already bot-only, so the
                # club is fully handed over the moment the creator is admin.
                if row["telegram_status"] == "ready" and _mark_club_connected(str(row["club_id"])):
                    try:
                        await bot.send_message(
                            user_id, "Your club group is ready — you are now an admin of it."
                        )
                    except TelegramError:
                        logger.warning("Could not confirm completed setup for club group %s", chat_id)
...
def reserve_join_policy(chat_id: int, user_id: int) -> bool | None:
    ...
    with get_db_session() as session:
        row = session.execute(
            text("""
                SELECT r.status, c.owner_user_id,
