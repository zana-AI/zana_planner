...
def _list_user_clubs(user_id: int) -> List[ClubSummary]:
    ...
    return [
        ClubSummary(
            ...
            leaderboard_time=str(row["leaderboard_time"]) if row["leaderboard_time"] else None,
...
async def get_club_leaderboard(
    ...
    try:
        ...
        result = compute_club_leaderboard(club_id, today=today, limit=limit)
        ...
        return ClubLeaderboardResponse(
            ...
            promise_count=result["promise_count"],
            average_score_percent=result["average_score_percent"],
            promises=[ClubLeaderboardPromiseSummary(**promise) for promise in result["promises"]],
            members=[ClubLeaderboardMember(**member) for member in result["members"]],
        )
...
async def update_club_settings(
    ...
    """Update club settings (reminder_time, language, timezone, leaderboard_time). Only club admins may call this."""
    try:
        clubs_repo = ClubsRepository()
        ...
        if body.timezone is not None:
            updates["timezone"] = body.timezone or None
        if body.leaderboard_time is not None:
            updates["leaderboard_time"] = body.leaderboard_time or None
